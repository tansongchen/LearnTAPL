...
let rec subtype ctx tyS tyT =
   if tyeqv ctx tyS tyT then () else
   let tyS = simplifyty ctx tyS in
   let tyT = simplifyty ctx tyT in
   match (tyS,tyT) with
     (_,TyTop) -> 
       ...
       let _ = List.for_all
         (fun (li,tyTi) -> 
            try let tySi = List.assoc li fS in
                (subtype ctx tySi tyTi; true)
            with Not_found -> raise (SubtypeFaliure (tyS, tyT)))
         fT in ()
   | (_,_) -> 
       raise (SubtypeFaliure (tyS, tyT))

let subtype_no_raise ctx tyS tyT =
  try let _ = subtype ctx tyS tyT in true
  with SubtypeFaliure (_, _) -> false

let rec join ctx tyS tyT =
  if subtype_no_raise ctx tyS tyT then tyT else 
  if subtype_no_raise ctx tyT tyS then tyS else
  let tyS = simplifyty ctx tyS in
  let tyT = simplifyty ctx tyT in
  match (tyS,tyT) with
    (TyRecord(fS), TyRecord(fT)) ->
...
and meet ctx tyS tyT =
  if subtype_no_raise ctx tyS tyT then tyS else 
  if subtype_no_raise ctx tyT tyS then tyT else 
  let tyS = simplifyty ctx tyS in
  let tyT = simplifyty ctx tyT in
  match (tyS,tyT) with
    (TyRecord(fS), TyRecord(fT)) ->
...
let rec typeof ctx t =
  ...
  | TmApp(fi,t1,t2) ->
      ...
      (match simplifyty ctx tyT1 with
          TyArr(tyT11,tyT12) ->
            (try let _ = subtype ctx tyT2 tyT11 in tyT12
            with SubtypeFaliure (tyS,tyT) -> pr "because "; printty ctx tyS; pr " is not a subtype of "; printty ctx tyT; pr "\n";
  ...
  | TmFalse(fi) -> 
      TyBool
  | TmIf(fi,t1,t2,t3) ->
      (try let _ = subtype ctx (typeof ctx t1) TyBool in join ctx (typeof ctx t2) (typeof ctx t3)
  ...
  | TmFix(fi, t1) ->
      ...
      (match simplifyty ctx tyT1 with
           TyArr(tyT11,tyT12) ->
             (try let _ = subtype ctx tyT12 tyT11 in tyT12
             with SubtypeFaliure (tyS,tyT) -> pr "because "; printty ctx tyS; pr " is not a subtype of "; printty ctx tyT; pr "\n"; error fi "result of body not compatible with domain")
         | _ -> error fi "arrow type expected")
  | TmString _ -> TyString
  | TmUnit(fi) -> TyUnit
  | TmAscribe(fi,t1,tyT) ->
     (try let _ = subtype ctx (typeof ctx t1) tyT in tyT
     with SubtypeFaliure (tyS,tyT) ->
       pr "because "; printty ctx tyS; pr " is not a subtype of "; printty ctx tyT; pr "\n"; error fi "body of as-term does not have the expected type")
  | TmFloat _ -> TyFloat
  | TmTimesfloat(fi,t1,t2) ->
      (try let _ = subtype ctx (typeof ctx t1) TyFloat;
      subtype ctx (typeof ctx t2) TyFloat in TyFloat
      with SubtypeFaliure (tyS,tyT) -> pr "because "; printty ctx tyS; pr " is not a subtype of "; printty ctx tyT; pr "\n"; error fi "argument of timesfloat is not a number")
  | TmZero(fi) ->
      TyNat
  | TmSucc(fi,t1) ->
      (try let _ = subtype ctx (typeof ctx t1) TyNat in TyNat
      with SubtypeFaliure (tyS,tyT) -> pr "because "; printty ctx tyS; pr " is not a subtype of "; printty ctx tyT; pr "\n"; error fi "argument of succ is not a number")
  | TmPred(fi,t1) ->
      (try let _ = subtype ctx (typeof ctx t1) TyNat in TyNat
      with SubtypeFaliure (tyS,tyT) -> pr "because "; printty ctx tyS; pr " is not a subtype of "; printty ctx tyT; pr "\n"; error fi "argument of pred is not a number")
  | TmIsZero(fi,t1) ->
      (try let _ = subtype ctx (typeof ctx t1) TyNat in TyBool
